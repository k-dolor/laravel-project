...
<div class="container-fluid p-0" style= "margin-left: 80px; padding-top: 20px;">
    <div class="card" style="position: relative; margin-top: 5px; margin-right: 10px; background-color: rgba(229, 225, 225, 0.8); color: #303030">
        <div class="card-header d-flex justify-content-between align-items-center" style="background-color: rgba(255, 255, 227, 0.785); color: rgb(32, 34, 35); border-bottom: none;">
            <h5 class="card-title" style="font-family: 'Bell MT'; font-size: 46px; margin-bottom: 0;">Edit Role</h5>
            <!--- VISIBLE BACK BUTTON--->
        ...
        <form action="/role/update/{{$role->gender_id}}" method = "post">
            @method('PUT')
            <div class="card-body">
            @csrf
            <div class="row h-100">
            <div class="col-md-12 mt-3 mb-5">
                <label for="role" style="font-family: 'Bell MT'; font-size: 40px;"> Role</label>
                <input type="text" class="form-control" name="role"
                id="role" id="role" value="{{$role->role}}" />
                @error('role') <p class="text-danger">{{$message}}</p>
                @enderror
            </div>
